...
export const Hero = () => {
  ...
  useEffect(() => {
    fetch("http://localhost:3001/api/media?where[alt][in]=Carousel-1,Carousel-2,Carousel-3")
      .then(res => res.json())
      .then(data => setCarouselImages(data.docs || []));
  }, []);
  ...
  return (
    <>
      ...
      {/* Carousel Başlangıcı - Tam Genişlik */}
      <div className="w-screen max-w-none px-0 mb-12 relative left-1/2 right-1/2 -mx-[50vw]">
        {/* Carousel başlığı tam ortada, görsellerin üstünde */}
        <div className="absolute inset-0 flex items-center justify-center z-20 pointer-events-none">
          ...
          <h1 className="text-white text-4xl md:text-5xl xl:text-7xl font-bold drop-shadow-lg text-center select-none">



            <WaveText text="İşiniz kimseye benzemez" delayPerChar={0.06} />
            <motion.p
        ...
        >
          {carouselImages.map((img, idx) => (
            <div className="flex flex-col h-full items-center w-full" key={idx}>
              <div className="w-full h-[700px] relative rounded-none overflow-hidden">
                <Image
                  src={img.url && img.url.startsWith("/") ? `http://localhost:3001${img.url}` : img.url}
                  alt={img.alt || ""}
                  fill
        ...
        <button
          onClick={handlePrev}
          className="group absolute left-8 top-1/2 -translate-y-1/2 z-10 flex items-center gap-4 px-6 py-4 text-white text-2xl font-bold rounded-full transition-all duration-300 hover:scale-110 animate-fadein overflow-hidden"
        >
          <FaChevronLeft className="text-3xl drop-shadow-lg group-hover:translate-x-3 transition-all duration-300 group-hover:animate-none" />
        </button>
        <button
          onClick={handleNext}
          className="group absolute right-8 top-1/2 -translate-y-1/2 z-10 flex items-center gap-4 px-6 py-4 text-white text-2xl font-bold rounded-full transition-all duration-300 hover:scale-110 animate-fadein overflow-hidden"
        >
          <FaChevronRight className="text-3xl drop-shadow-lg group-hover:-translate-x-3 transition-all duration-300 group-hover:animate-none" />
